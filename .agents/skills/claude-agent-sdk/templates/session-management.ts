...
async function sequentialDevelopment() {
  ...
  const session = await startSession("Create a user authentication system with JWT");
...
async function explorationAndDecision() {
  ...
  const mainSession = await startSession("Design a payment processing system");
...
async function multiUserCollaboration() {
  ...
  const sessionA = await startSession("Implement user profile page with avatar, bio, and settings");
