...
Block deployments if critical issues found.`,
        ...
        "performance-agent": {
          description:
            "Analyzes performance metrics, identifies bottlenecks, and suggests optimizations",
...
Identify bottlenecks and optimization opportunities.`,
      ...
      canUseTool: async (toolName, input) => {
        ...
        if (
          input.command?.includes("deploy --production") ||
...
async function main() {
  try {
    ...
    await runDevOpsAgent("Deploy version 2.5.0 to production with full validation and monitoring");
